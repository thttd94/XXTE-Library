...
local EVENT_IMGS = {
 RES_DIR .. "Event1_crop.png",
 RES_DIR .. "Event2_crop.png",
 RES_DIR .. "Event1.png",
 RES_DIR .. "Event2.png"
}

local EVENT_SCALE_IMGS = {
 RES_DIR .. "Event1_crop_s080.png",
 RES_DIR .. "Event1_crop_s090.png",
 RES_DIR .. "Event1_crop_s100.png",
 RES_DIR .. "Event1_crop_s110.png",
 RES_DIR .. "Event1_crop_s120.png",
 RES_DIR .. "Event2_crop_s080.png",
 RES_DIR .. "Event2_crop_s090.png",
 RES_DIR .. "Event2_crop_s100.png",
 RES_DIR .. "Event2_crop_s110.png",
 RES_DIR .. "Event2_crop_s120.png",
 RES_DIR .. "Event1_s080.png",
 RES_DIR .. "Event1_s090.png",
 RES_DIR .. "Event1_s100.png",
 RES_DIR .. "Event1_s110.png",
 RES_DIR .. "Event1_s120.png",
 RES_DIR .. "Event2_s080.png",
 RES_DIR .. "Event2_s090.png",
 RES_DIR .. "Event2_s100.png",
 RES_DIR .. "Event2_s110.png",
 RES_DIR .. "Event2_s120.png"
}
...
function tapByAnyImage(imgList, sim, x1, y1, x2, y2, offsetX, offsetY)
 offsetX = offsetX or 0
 offsetY = offsetY or 0

 local ok, x, y = findAnyImage(imgList, sim, x1, y1, x2, y2)
 if ok then
  touch.tap(x + offsetX, y + offsetY)
  return true, x, y
 end
...
function handlePopup8Fixed()
 ...
 if (not byAgree) and (not byCheck) then
  return false
 end
 ...
 touch.tap(370, 807)
 waitPhase(700)
 if hasAgreeContinuePopupByColor() or findAnyImage(POPUP_8_CHECK, 84, 0, 0, 750, 1334) then
  status("Popup 8 còn")
 else
  status("Popup 8 hết")
 end
...
function hasAppStoreInstallError()
 return screen.is_colors(APPSTORE_INSTALL_ERROR, 88)
end

function handleAppStoreInstallError()
 if not hasAppStoreInstallError() then
  return false
 end

 phase("Lỗi cài TikTok")
 touch.tap(337, 685)
 waitPhase(1500)
 return true
end
...
function closeTikTok()
 phase("Đóng TikTok")
 app.close(TIKTOK_BUNDLE)
 waitPhase(1500)
end

function closeAppManager()
 phase("Đóng AppManager")
 app.close(APPMANAGER_BUNDLE)
 waitPhase(1500)
end
...
function confirmNoPopupSlow()
 waitPhase(300)
 if findAnyImage(POPUP_1_CHECK, 88, 0, 0, 750, 1334) then return false end

 waitPhase(300)
 if findAnyImage(POPUP_2_CHECK, 88, 0, 0, 750, 1334) then return false end

 waitPhase(300)
 if findAnyImage(POPUP_3_CHECK, 88, 0, 0, 750, 1334) then return false end

 waitPhase(300)
 if findAnyImage(POPUP_4_CHECK, 88, 0, 0, 750, 1334) then return false end

 waitPhase(300)
 if findAnyImage(POPUP_5_CHECK, 88, 0, 0, 750, 1334) then return false end

 waitPhase(300)
 if findAnyImage(POPUP_6_CHECK, 88, 0, 0, 750, 1334) then return false end

 waitPhase(300)
 if hasAgreeContinuePopupByColor() then return false end

 waitPhase(300)
 if findAnyImage(POPUP_8_CHECK, 84, 0, 0, 750, 1334) then return false end

 waitPhase(300)
 if findAnyImage(POPUP_9_CHECK, 84, 0, 0, 750, 1334) then return false end

 waitPhase(300)
 if hasPopup7ByColor() and (not hasAgreeContinuePopupByColor()) and (not findAnyImage(POPUP_8_CHECK, 84, 0, 0, 750, 1334)) then return false end

 return true
end

function waitPopupStableFast()
 waitPhase(2500)
end
...
function handlePopupByImages(name, checkImgs, tapImg, simCheck, simTap, x1, y1, x2, y2)
 ...
 if tapped then
  waitPhase(500)
  if findAnyImage(checkImgs, simCheck or 88, x1, y1, x2, y2) then
   status(name .. " còn")
  else
   status(name .. " hết")
  end
 ...
 local fallbackOffsetX = 0
 local fallbackOffsetY = 0
 if name == "Popup 8" or name == "Popup 9" then
  fallbackOffsetX = 120
  fallbackOffsetY = 120
 end

 status(name .. " fallback")
 touch.tap(checkX + fallbackOffsetX, checkY + fallbackOffsetY)
 waitPhase(500)
 if findAnyImage(checkImgs, simCheck or 88, x1, y1, x2, y2) then
  status(name .. " còn")
 else
...
function ensurePopup7Cleared(maxRounds)
 ...
 for i = 1, maxRounds do
  if handlePopup8Fixed() then
   waitPhase(500)
  end

  if handlePopupByImages("Popup 9", POPUP_9_CHECK, POPUP_9_TAP, 84, 84, 0, 0, 750, 1334) then
   waitPhase(500)
  end

  if not hasAgreeContinuePopupByColor() and (not findAnyImage(POPUP_8_CHECK, 84, 0, 0, 750, 1334)) and (not findAnyImage(POPUP_9_CHECK, 84, 0, 0, 750, 1334)) and (not hasPopup7ByColor()) then
   return true
  end
...
function handleOnePopup()
 status("Rà Popup 1")
 if handlePopupByImages("Popup 1", POPUP_1_CHECK, POPUP_1_TAP, 88, 88, 0, 0, 750, 1334) then return true end

 status("Rà Popup 2")
 if handlePopupByImages("Popup 2", POPUP_2_CHECK, POPUP_2_TAP, 88, 88, 0, 0, 750, 1334) then return true end

 status("Rà Popup 3")
 if handlePopupByImages("Popup 3", POPUP_3_CHECK, POPUP_3_TAP, 88, 88, 0, 0, 750, 1334) then return true end

 status("Rà Popup 4")
 if handlePopupNoInternet() then return true end

 status("Rà Popup 5")
 if handlePopupByImages("Popup 5", POPUP_5_CHECK, POPUP_5_TAP, 88, 88, 0, 0, 750, 1334) then return true end

 status("Rà Popup 6")
 if handlePopupByImages("Popup 6", POPUP_6_CHECK, POPUP_6_TAP, 88, 88, 0, 0, 750, 1334) then return true end

 status("Rà Popup 8")
 if handlePopup8Fixed() then return true end

 status("Rà Popup 9")
 if handlePopupByImages("Popup 9", POPUP_9_CHECK, POPUP_9_TAP, 84, 84, 0, 0, 750, 1334) then return true end

 status("Rà Popup 7")
 if handlePopupSwipeUp() then return true end
...
function findEventByColor(sim)
 ...
 for i = 1, #EVENT_COLOR_PATTERNS do
  ...
  if x ~= -1 then
   return true, x + (p.offsetX or 0), y + (p.offsetY or 0), p.name
  end
 end
...
function findEventFast()
 local ok, x, y, tag = findEventByColor(95)
 if ok then
  return true, x, y, tag
 end

 ok, x, y, tag = findEventByColor(93)
 if ok then
  return true, x, y, tag
 end

 ok, x, y, tag = findEventByColor(90)
 if ok then
  return true, x, y, tag
 end
...
function tapEventFast()
 ...
 if hit == "event_gold_m" or hit == "event_gold_s" or hit == "event_gold_l" then
  touch.tap(x, y)
 else
  touch.tap(x + 30, y + 30)
 end
...
function patrolPopupDuringEvent()
 if hasPopupActive() then
  phase("Tuần tra popup")
  status("Đang rà popup")
  waitPopupStableFast()
  if handleOnePopup() then
   status("Xử lý popup xong 1 nhịp")
   waitPhase(350)
  else
   status("Có popup nhưng chưa xử lý được")
   waitPhase(200)
  end
  return true
 end
...
function waitEventInWindow(seconds)
 local start = os.time()
 local lastShown = -1
 local next_swipe_at = 20

 if not popup_cleared then
  phase("Chưa sạch popup")
  return false
 end
...
function runEventFlowWithRestart()
 if not popup_cleared then
  phase("Chưa sạch popup")
  return false
 end

 return waitEventInWindow(60)
end
...
function backupTikTokInAppManager()
 phase("Bắt đầu backup")

 closeTikTok()
 closeAppManager()
 openAppManager()

 local row_ok = waitTapImage(TIKTOK_ROW_IMG, "Tìm TikTok", 30, 82, 0, 150, 750, 1100)
 if not row_ok then
  return false
 end

 local backup_ok = waitTapImage(TIKTOK_BACKUP_IMG, "Tìm Backup", 30, 82, 0, 150, 750, 1334)
 if not backup_ok then
  return false
 end

 local backup_data_ok = waitTapImage(TIKTOK_BACKUPDATA_IMG, "Xác nhận data", 30, 82, 0, 150, 750, 1334)
 if not backup_data_ok then
  return false
 end

 local backuping_appear = waitImageAppear(TIKTOK_BACKUPING_IMG, "Đợi popup backup", 30, 82, 0, 150, 750, 1334)
 if not backuping_appear then
  return false
 end

 local backuping_done = waitImageDisappear(TIKTOK_BACKUPING_IMG, "Backup", 1800, 82, 0, 150, 750, 1334)
 if not backuping_done then
  return false
 end
...
function openTikTokStoragePage()
 phase("Mở Storage")
 app.close("com.apple.Preferences")
 waitPhase(1500)
 app.open_url("prefs:root=General&path=STORAGE_MGMT/com.ss.iphone.ugc.Ame")
 waitPhase(12000)
 app.open_url("prefs:root=General&path=STORAGE_MGMT/com.ss.iphone.ugc.Ame")
 waitPhase(5000)
end
...
function stage1DeleteTikTokOnce()
 ...
 if installed_before == false then
  status("Không có TikTok")
  waitCountdown(60000, "Đợi AppStore")
  return true
 end

 phase("Gỡ TikTok")
 local ok = app.uninstall(TIKTOK_BUNDLE)
 waitPhase(3000)
 ...
 while os.time() - start_wait < 120 do
  ...
  if installed_now == nil then
   if not app.is_running(TIKTOK_BUNDLE) then
    phase("TikTok đã tắt")
   end
  end
...
function stage2DownloadTikTokOnce()
 ...
 local tiktok_store_url = "https://apps.apple.com/jp/app/tiktok-%E3%83%86%E3%82%A3%E3%83%83%E3%82%AF%E3%83%88%E3%83%83%E3%82%AF/id1235601864"
 ...
 local function openStoreTikTok()
  ...
  app.close("com.apple.AppStore")
  waitPhase(1500)
  app.open_url(tiktok_store_url)
  waitPhase(6000)
 end

 local function hasAnyCheckTai()
  if findImage(CHECK_TAI_1, 82, 0, 0, 750, 1334) then
   return true
  end
  if findImage(CHECK_TAI_2, 82, 0, 0, 750, 1334) then
   return true
  end
  if findImage(CHECK_TAI_3, 82, 0, 0, 750, 1334) then
   return true
  end
  return false
 end
 ...
 local function hasCloud()
  local ok = findImage(CLOUD_IMG, 82, CLOUD_X1, CLOUD_Y1, CLOUD_X2, CLOUD_Y2)
  return ok == true
 end
 ...
 local function hasCannot()
  local ok = findImage(CANNOT_IMG, 82, CANNOT_X1, CANNOT_Y1, CANNOT_X2, CANNOT_Y2)
  if ok then
   return true
  end
  return false
 ...
 local function tapRetryOnce()
  local ok, x, y = findImage(RETRY_IMG, 82, RETRY_X1, RETRY_Y1, RETRY_X2, RETRY_Y2)
  if ok then
   phase("Bấm retry")
   touch.tap(x + 20, y + 20)
   waitPhase(1000)
 ...
 local function handleCannotAndRetry(window_sec)
  ...
  while os.time() - start < window_sec do
   ...
   if hasCannot() then
    ...
    if not retried then
     if tapRetryOnce() then
      retried = true
      phase("Đã bấm retry, chờ")
      waitPhase(4000)
     end
    else
    end
 ...
 local function waitCannotRecovery()
  ...
  for i = 1, #retry_schedule do
   ...
   if step > 0 then
    if handleCannotAndRetry(step) then
     return true
    end
   end

   phase("Mở lại AppStore")
   openStoreTikTok()
   if hasAnyCheckTai() then
    return true
   end
 ...
 local function tapCloudOnce()
  local ok, x, y = findImage(CLOUD_IMG, 82, CLOUD_X1, CLOUD_Y1, CLOUD_X2, CLOUD_Y2)
  if ok then
   phase("Bấm cloud")
   touch.tap(x + 20, y + 20)
   waitPhase(1000)
 ...
 while os.time() - start_stage2 < 10800 do
  local elapsed = os.time() - start_stage2
  local remain = 10800 - elapsed
  if download_started and remain ~= lastShown then
   phaseProgress(remain)
   lastShown = remain
  end
  ...
  if hasCannot() then
   phase("Không có kết nối")
   if waitCannotRecovery() then
    phase("Ra lại bước tải")
   else
    return false
   end
  else
  ...
  if has_unable then
   phase("Unable")
   waitTapImage(UNABLE_OK_IMG, "Bấm unable ok", 3, 82, 0, 0, 750, 1334)
   waitPhase(2000)
   if hasCloud() and (os.time() - last_cloud_tap_at >= 2) then
    if tapCloudOnce() then
     last_cloud_tap_at = os.time()
     download_started = true
     phase("Chờ OPEN")
  ...
  if hasCloud() and download_started and (os.time() - last_cloud_tap_at >= cloud_tap_cooldown) then
   phase("Cloud xuất hiện lại")
   if tapCloudOnce() then
    last_cloud_tap_at = os.time()
    phase("Chờ OPEN")
    waitPhase(8 * 1000)
...
function stage3OpenTikTokOnce()
 closeTikTok()
 openTikTok()
 phase("Đợi sau mở TikTok")
 waitPhase(20000)

 local popup_ok = runTikTokPopupFlow(120)
 ...
 local event_ok = runEventFlowWithRestart()
...
function backupTikTokInAppManagerNoStop()
 ...
 local row_ok = waitTapImage(TIKTOK_ROW_IMG, "Tìm TikTok", 30, 82, 0, 150, 750, 1100)
 if not row_ok then return false end

 local backup_ok = waitTapImage(TIKTOK_BACKUP_IMG, "Tìm Backup", 30, 82, 0, 150, 750, 1334)
 if not backup_ok then return false end

 local backup_data_ok = waitTapImage(TIKTOK_BACKUPDATA_IMG, "Xác nhận data", 30, 82, 0, 150, 750, 1334)
 if not backup_data_ok then return false end

 local backuping_appear = waitImageAppear(TIKTOK_BACKUPING_IMG, "Đợi popup backup", 30, 82, 0, 150, 750, 1334)
 if not backuping_appear then return false end

 local backuping_done = waitImageDisappear(TIKTOK_BACKUPING_IMG, "Backup", 1800, 82, 0, 150, 750, 1334)
 if not backuping_done then return false end

 phase("Backup xong")
 return true
end

function stage4BackupTikTokOnce()
 closeTikTok()
 closeAppManager()
 return backupTikTokInAppManagerNoStop()
end

phase("Khởi động v" .. SCRIPT_VERSION)
waitPhase(1500)
...
while true do
 ...
 if current_stage == 1 then
  clearStage3Watch()
  phase("Stage 1")
  if stage1DeleteTikTokOnce() then
   current_stage = 2
  else
   closeTikTok()
   app.close("com.apple.Preferences")
 ...
 elseif current_stage == 2 then
  clearStage3Watch()
  phase("Stage 2")
  local stage2_result = stage2DownloadTikTokOnce()
  if stage2_result == true then
   current_stage = 3
   beginStage3Watch()
  elseif stage2_result == "restart_stage1" then
   current_stage = 1
   clearStage3Watch()
   closeTikTok()
   closeAppManager()
 ...
 elseif current_stage == 3 then
  if not stage3_started_at then
   beginStage3Watch()
  end
  phase("Stage 3")
  if stage3OpenTikTokOnce() then
   clearStage3Watch()
   current_stage = 4
  else
   if shouldRestartFromStage3() then
 ...
 elseif current_stage == 4 then
  clearStage3Watch()
  phase("Stage 4")
  if stage4BackupTikTokOnce() then
   tapBackupOk()
   break
  else
   closeTikTok()
