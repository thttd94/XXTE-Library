screen.init(0)
local __WEBVIEW_STATUS_TEXT = "Xuất TK Đang chạy ..."
...
</style></head><body><div id="bar">Xuất TK Đang chạy ...</div></body></html>
]]
...
local SCRIPT_VERSION = "Session Export 4.0 Tiktok+Lite"
mkdirp(TIKTOK_DIR); mkdirp(LITE_DIR)

local function exportPath(label, src, dst)
  local data=readAll(src)
  if not data then status(label .. " read lỗi", 1500); return false end
...
if mainDir then
  if exportPath("Tiktok cookies", mainDir.."/Library/Cookies/Cookies.binarycookies", TIKTOK_DIR.."/Cookies.binarycookies") then okMain=okMain+1 end
  if exportPath("Tiktok archiver", mainDir.."/Documents/ttaccountSDKUserInfo.archiver", TIKTOK_DIR.."/ttaccountSDKUserInfo.archiver") then okMain=okMain+1 end
  if exportPath("Tiktok plist", mainDir.."/Library/Preferences/com.ss.iphone.ugc.Ame.plist", TIKTOK_DIR.."/com.ss.iphone.ugc.Ame.plist") then okMain=okMain+1 end
else
  status("Tiktok data miss", 1200)
end
local sec=findAppGroupFile("sec_uid_storage_file")
if sec and exportPath("Tiktok sec_uid", sec, TIKTOK_DIR.."/sec_uid_storage_file") then okMain=okMain+1 end

local okLite=0
local liteDir=findDataDir("com.ss.iphone.ugc.tiktok.lite.plist")
if liteDir then
  if exportPath("Lite cookies", liteDir.."/Library/Cookies/Cookies.binarycookies", LITE_DIR.."/Cookies.binarycookies") then okLite=okLite+1 end
  if exportPath("Lite archiver", liteDir.."/Documents/ttaccountSDKUserInfo.archiver", LITE_DIR.."/ttaccountSDKUserInfo.archiver") then okLite=okLite+1 end
  if exportPath("Lite plist", liteDir.."/Library/Preferences/com.ss.iphone.ugc.tiktok.lite.plist", LITE_DIR.."/com.ss.iphone.ugc.tiktok.lite.plist") then okLite=okLite+1 end
else
...
if sys and type(sys.alert)=="function" then
  sys.alert("Xuất TK OK\nTiktok: "..tostring(okMain).."/4\nTiktok Lite: "..tostring(okLite).."/3")
else
  status("Xuất TK OK - Tiktok "..tostring(okMain).."/4 Lite "..tostring(okLite).."/3", 10000)
end
